...
export type SyllableType = 'compound' | 'integrated';

// 单个音节
export interface PinyinSyllable {
  id: string;           // 唯一标识，如 "ba1", "zhi"
  pinyin: string;       // 拼音（无声调），如 "ba", "zhi"
  marked: string;       // 带声调拼音，用于显示和 TTS，如 "bā"
  characters: string[]; // 关联汉字（1-2个常用字）
...
// ─── 整体认读音节 ───────────────────────────────────────────
export const INTEGRATED_READINGS = [
  'zhi', 'chi', 'shi', 'ri',
  'zi', 'ci', 'si',
  'yi', 'wu', 'yu',
  'ye', 'yue', 'yuan',
...
const COMPOUND_ENTRIES: RawEntry[] = [
  ...
  ['xiu', 'xiū', '修'], ['xiu', 'xiù', '秀'],
  // ie
  ['bie', 'bié', '别'], ['bie', 'biàn', '变'], ['pie', 'piē', '撇'], ['mie', 'miè', '灭'], ['mie', 'miè', '蔑'],
  ...
  ['zeng', 'zēng', '增'], ['zeng', 'céng', '曾'], ['ceng', 'céng', '层'], ['ceng', 'céng', '曾'],
  ['zheng', 'zhèng', '正'], ['zheng', 'zhēng', '争'], ['cheng', 'chéng', '成'], ['cheng', 'chéng', '城'],
...
const INTEGRATED_MARKED: Record<string, [string, string]> = {
  zhi: ['zhī', '知'], zhi2: ['zhī', '之'],
  chi: ['chī', '吃'], chi2: ['chí', '迟'],
  shi: ['shì', '是'], shi2: ['shí', '时'],
  ri: ['rì', '日'],
  zi: ['zì', '字'], zi2: ['zǐ', '子'],
  ci: ['cì', '次'], ci2: ['cǐ', '此'],
  si: ['sì', '四'], si2: ['sī', '思'],
  ...
  yu: ['yǔ', '与'], yu2: ['yǔ', '雨'],
  ye: ['yě', '也'], ye2: ['yè', '夜'],
  yue: ['yuè', '月'], yue2: ['yuē', '约'],
  yuan: ['yuán', '元'], yuan2: ['yuán', '原'],
  yin: ['yīn', '音'], yin2: ['yín', '银'],
  yun: ['yún', '云'], yun2: ['yùn', '运'],
  ying: ['yīng', '英'], ying2: ['yīng', '应'],
};
...
function computeDifficulty(pinyin: string): Difficulty {
  ...
  let matched = '';
  for (const f of [...compoundFinals, ...nasalFinals, ...simpleFinals]) {
...
const syllableMap = new Map<string, PinyinSyllable>();

// 整体认读 → L3
const integratedKeys = Object.keys(INTEGRATED_MARKED);
for (const key of integratedKeys) {
  const [marked, char] = INTEGRATED_MARKED[key];
  const base = key.replace(/2$/, '');
  const id = `${base}_${marked.replace(/[^a-z]/g, '')}`; // e.g. zhi_zhi
  if (!syllableMap.has(id)) {
    syllableMap.set(id, {
      id,
      pinyin: base,
      marked,
      characters: [char],
      difficulty: 'L3',
      syllableType: 'integrated',
    });
  } else {
    const existing = syllableMap.get(id)!;
...
// 声母+韵母组合
for (const [pinyin, marked, char1, char2] of COMPOUND_ENTRIES) {
  // 构造唯一 id：拼音_汉字，避免同拼音不同声调冲突，如 ba_八 vs ba_爸
  const id = `${pinyin}_${char1}`;

  const difficulty = computeDifficulty(pinyin);

  if (syllableMap.has(id)) {
    // 已存在，追加汉字
    const existing = syllableMap.get(id)!;
    if (char2 && !existing.characters.includes(char2)) {
      existing.characters.push(char2);
    }
  } else {
    syllableMap.set(id, {
      id,
      pinyin: pinyin,
      marked,
      characters: char2 ? [char1, char2] : [char1],
      difficulty,
      syllableType: 'compound',
    });
  }
}

export const PINYIN_DATA: PinyinSyllable[] = Array.from(syllableMap.values());

// ─── 按难度过滤 ─────────────────────────────────────────────

export const getByDifficulty = (level: 1 | 2 | 3 | 4): PinyinSyllable[] =>
  PINYIN_DATA.filter((s) => s.difficulty === `L${level}`);
...
export interface LevelConfig {
  ...
  optionCount: number;
  hasCrossLevelDistractors: boolean;
}

export const LEVEL_CONFIGS: LevelConfig[] = [
  { level: 1, name: '声母关', description: '单韵母拼读', difficulty: 'L1', emoji: '🌱', colorClass: 'bg-green-500/20', textColorClass: 'text-green-400', optionCount: 4, hasCrossLevelDistractors: false },
  { level: 2, name: '韵母关', description: '复合韵母拼读', difficulty: 'L2', emoji: '🌿', colorClass: 'bg-yellow-500/20', textColorClass: 'text-yellow-400', optionCount: 4, hasCrossLevelDistractors: false },
  { level: 3, name: '整体认读', description: '16个整体认读音节', difficulty: 'L3', emoji: '🌳', colorClass: 'bg-red-500/20', textColorClass: 'text-red-400', optionCount: 4, hasCrossLevelDistractors: true },
  { level: 4, name: '拼读关', description: '鼻韵母综合', difficulty: 'L4', emoji: '🏆', colorClass: 'bg-purple-500/20', textColorClass: 'text-purple-400', optionCount: 4, hasCrossLevelDistractors: true },
];

// ─── 统计 ─────────────────────────────────────────────────

export const STATS = {
  totalSyllables: PINYIN_DATA.length,
  totalCharacters: PINYIN_DATA.reduce((acc, s) => acc + s.characters.length, 0),
  byDifficulty: {
    L1: PINYIN_DATA.filter((s) => s.difficulty === 'L1').length,
    L2: PINYIN_DATA.filter((s) => s.difficulty === 'L2').length,
    L3: PINYIN_DATA.filter((s) => s.difficulty === 'L3').length,
    L4: PINYIN_DATA.filter((s) => s.difficulty === 'L4').length,
  },
};
