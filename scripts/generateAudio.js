#!/usr/bin/env node
/**
 * generateAudio.js - 使用 macOS say Tingting 音色生成音频文件
 * 
 * 用法: node scripts/generateAudio.js
 */
...
const DATA_FILE = join(ROOT, 'src', 'data', 'pinyinData.ts');

mkdirSync(AUDIO_DIR, { recursive: true });

// 手动解析 pinyinData.ts 中的 COMPOUND_ENTRIES 和 INTEGRATED_MARKED
// 这是最简单的方法，避免复杂的 TypeScript 解析
...
function makeId(marked, base) {
  const toneStripped = marked.replace(/[áàǎāēèěēīìǐīōòǒōūùǔūüǜǚǜǘ]/g, (m) => {
    const map = { 'ā':'a','á':'a','ǎ':'a','à':'a','ē':'e','é':'e','ě':'e','è':'e','ī':'i','í':'i','ǐ':'i','ì':'i','ō':'o','ó':'o','ǒ':'o','ò':'o','ū':'u','ú':'u','ǔ':'u','ù':'u','ü':'v','ǖ':'v','ǘ':'v','ǚ':'v','ǜ':'v' };
    return map[m] || m;
  });
...
// 收集所有唯一音节
const seen = new Map();
for (const [base,,char1,char2] of COMPOUND_ENTRIES) {
  const marked = base; // 第二个元素是 marked
  if (!seen.has(marked)) {
    seen.set(marked, { marked, id: makeId(marked, base), char: char1 });
  }
}
// Actually the array format is [pinyin, marked, char1, char2?]
const seen2 = new Map();
for (const entry of COMPOUND_ENTRIES) {
  const [pinyin, marked, char1, char2] = entry;
  if (!seen2.has(marked)) {
    seen2.set(marked, { marked, id: makeId(marked, pinyin) });
  }
}

// Need to use the correct array format
const allSyllables = [];
const seenMarks = new Set();

// Compound entries: [pinyin, marked, char1, char2?]
for (const entry of COMPOUND_ENTRIES) {
  const marked = entry[1];
  if (!seenMarks.has(marked)) {
    seenMarks.add(marked);
    allSyllables.push({ marked, id: makeId(marked, entry[0]) });
  }
}

// Integrated
for (const entry of INTEGRATED_MARKED) {
  const marked = entry[0];
  if (!seenMarks.has(marked)) {
    seenMarks.add(marked);
    const base = marked.replace(/[àáǎāèéěēìíǐīòóǒōùúǔūüǜǚǜǘ]/g, '').replace('ü', 'v');
    allSyllables.push({ marked, id: makeId(marked, base) });
  }
}

const existing = new Set(readdirSync(AUDIO_DIR).filter(f => f.endsWith('.m4a')));
const toGenerate = allSyllables.filter(s => !existing.has(`${s.id}.m4a`));

console.log(`已有: ${existing.size} | 需生成: ${toGenerate.length} | 总计: ${allSyllables.length}`);
...
function generateNext() {
  while (active < CONCURRENCY && index < toGenerate.length) {
    const item = toGenerate[index++];
    active++;
    generateOne(item).then(() => {
      active--;
      generateNext();
    });
  }
...
function generateOne(item) {
  return new Promise((resolve) => {
    ...
    const filepath = join(AUDIO_DIR, `${id}.m4a`);
    try {
      // 直接用 execSync，marked 作为 stdin
...
function update() {
  if (done % 20 === 0 || done === toGenerate.length) {
    const pct = ((done / toGenerate.length) * 100).toFixed(0);
    const elapsed = ((Date.now() - start) / 1000).toFixed(0);
    process.stdout.write(`\r[${pct}%] ${done}/${toGenerate.length} (${errors}错) ${elapsed}s   `);
  }
}
